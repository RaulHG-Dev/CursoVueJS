  ...
  "packages/vue/src/bindings.ts"() {
    vueBindings = function vueBindings2(node) {
      ...
      const isComplete = vue.computed(() => {
        return context && hasValidation.value ? isValid.value && !hasErrors.value : context.state.dirty && !utils.empty(context.value);
      });
          ...
          if (!className && typeof property === "string") {
            if (!utils.has(target, property) && !property.startsWith("__v")) {
              ...
              observedNode.watch((node2) => {
                ...
                const globalConfigClasses = node2.config.classes ? core.createClasses(property, node2, node2.config.classes[property]) : {};
                const classesPropClasses = core.createClasses(
                  property,
                  node2,
                  node2.props[`_${property}Class`]
                );
                const sectionPropClasses = core.createClasses(
                  property,
                  node2,
                  node2.props[`${property}Class`]
                );
                className = core.generateClassList(
                  node2,
                  property,
                  rootClasses,
                  globalConfigClasses,
        ...
        handlers: {
          blur: (e) => {
            if (!node)
              return;
            node.store.set(
              /* @__PURE__ */ core.createMessage({ key: "blurred", visible: false, value: true })
            );
          ...
          touch: () => {
            ...
            if (!isDirty && !doCompare)
              return;
            node.store.set(
              /* @__PURE__ */ core.createMessage({ key: "dirty", visible: false, value: isDirty })
            );
          },
          DOMInput: (e) => {
      ...
      node.on("commit", ({ payload }) => {
        ...
        if (isComplete && node.type === "input" && hasErrors.value && !utils.undefine(node.props.preserveErrors)) {
          node.store.filter(
...
var init_defaultConfig = __esm({
  "packages/vue/src/defaultConfig.ts"() {
    ...
    exports.defaultConfig = (options = {}) => {
      ...
      const i18n$1 = i18n.createI18nPlugin(
        utils.extend({ en: i18n.en, ...locales || {} }, messages3)
      );
      ...
      return utils.extend(
        {
          plugins: [library, themePlugin, exports.bindings, i18n$1, validation$1],
...
function parseSchema(library, schema, memoKey) {
    ...
    if (unparsedAttrs) {
      ...
      for (let attr in unparsedAttrs) {
        ...
        if (attr.startsWith(raw)) {
          ...
          getValue2 = provider(core.compile(value));
  ...
  function parseNode(library2, _node) {
    ...
    if (core.isDOM(node)) {
      ...
      if (typeof node.$cmp === "string") {
        if (utils.has(library2, node.$cmp)) {
    ...
    if ("for" in node && node.for) {
      ...
      const getValues = typeof values === "string" && values.startsWith("$") ? provider(core.compile(values)) : () => values;
      iterator = [
        getValues,
        node.for[0],
  ...
  function createInstance(providerCallback, key) {
    ...
    const [render, compiledProviders] = utils.has(memo, memoKey) ? memo[memoKey] : [createElements(library, schema), providers];
    if (!isServer2) {
    ...
    (requirements, hints = {}) => {
      return requirements.reduce((tokens, token3) => {
        if (token3.startsWith("slots.")) {
          ...
          const hasSlot = () => data.slots && utils.has(data.slots, slot) && typeof data.slots[slot] === "function";
          if (hints.if) {
...
function useInput(props, context, options = {}) {
  ...
  const __root = vue.inject(rootSymbol, vue.ref(isBrowser ? document : void 0));
  ...
  const value = props.modelValue !== void 0 ? props.modelValue : utils.cloneAny(context.attrs.value);
  function createInitialProps() {
    ...
    for (const propName in propValues) {
      ...
      initialProps2[utils.camel(propName)] = propValues[propName];
    }
  ...
  vue.watchEffect(() => {
    const messages3 = (props.errors ?? []).map(
      (error3) => /* @__PURE__ */ core.createMessage({
        key: utils.slugify(error3),
        type: "error",
        value: error3,
        meta: { source: "prop" }
      })
  ...
  if (isVModeled) {
    vue.watch(
      ...
      (value2) => {
        if (!utils.eq(clonedValueBeforeVmodel, value2)) {
...
function createInput(schemaOrComponent, definitionOptions = {}, sectionsSchema = {}) {
  ...
  definition3.schema = inputs.useSchema(schema || "Schema undefined", sectionsSchema);
...
var messages = inputs.createSection("messages", () => ({
  $el: "ul",
  if: "$fns.length($messages)"
}));
var message = inputs.createSection("message", () => ({
  $el: "li",
...
var FormKitMessages = /* @__PURE__ */ vue.defineComponent({
  ...
  setup(props, context) {
    ...
    vue.watch(
      node,
      () => {
        if (node.value?.context && !utils.undefine(props.defaultPosition)) {
...
function useFormKitContextById(id, effect) {
  const context = vue.ref();
  const targetNode = core.getNode(id);
  if (targetNode)
    context.value = targetNode.context;
  if (!targetNode) {
    const receipt = core.watchRegistry(id, ({ payload: node }) => {
      if (node) {
        context.value = node.context;
        core.stopWatch(receipt);
        if (effect)
          effect(context.value);
      }
    });
...
var summary = inputs.createSection("summary", () => ({
  $el: "div",
  attrs: {
    "aria-live": "polite"
  }
}));
var summaryInner = inputs.createSection("summaryInner", () => ({
  $el: "div",
  if: "$summaries.length && $showSummaries"
}));
var messages2 = inputs.createSection("messages", () => ({
  $el: "ul",
  if: "$summaries.length && $showSummaries"
}));
var message2 = inputs.createSection("message", () => ({
  $el: "li",
...
var summaryHeader = inputs.createSection("summaryHeader", () => ({
  $el: "h2",
  attrs: {
    id: "$id"
  }
}));
var messageLink = inputs.createSection("messageLink", () => ({
  $el: "a",
  attrs: {
    id: "$summary.key",
    href: '$: "#" + $summary.id',
...
Object.defineProperty(exports, "changeLocale", {
  enumerable: true,
  get: function () { return i18n.changeLocale; }
});
